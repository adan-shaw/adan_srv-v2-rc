...
int main (int argc, int argv[])
{
	...
	for (;;)											//输入控制loop
	{
		...
		{
		case 1:
			if (s_val->srv_sig != 0)	//服务已经开启/拒绝开始服务
			{
			...
			break;
		case 2:
			if (s_val->srv_sig == 0)	//服务没有启动/简单停止
			{;
				exit (0);
			}
			//服务启动,全部杀停--经常报错就这里
			quit_main (s_val);
			exit (0);									//调用退出函数--这里要确定能回收所有资源--全部kill
	...
	printf ("the assrvxx server has stop,input any key to quit\n");
	printf ("******************************************\n");
	scanf ("%d", &tmp_loop);
	exit (0);
}
