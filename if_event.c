...
{																//epoll 临时变量
	...
	//继承IO 任务队列
	struct aq_info *pm_recv;
	pm_recv = s_val->pm_recv;
	//
	...
	{
		for (tmp = 0; tmp < wait_fds; tmp++)	//用循环体处理所有被触发的fd
		{														//listen fd 发生事件
			if (evs[tmp].data.fd == s_val->sfd_li && s_val->cur_fds < MAXEPOLL)	//!> if是监听端口有事
			{
				...
				{
					s_val->err_li++;
					printf ("listen pthread add_epoll() fail, error count: %d", s_val->err_li);
				}												//失败错误次数+1
			}
			else											//当客户端有IO数据
			{													//直接从epoll 中移除client
		...
		//执行批量压入队列
		if (aq_push (pm_recv, as_size) != pm_recv->aq_push_ok)
		{
			s_val->err_li += as_size;
			printf ("listen pthread aq_push in queue error, errno: %d\n", errno);
